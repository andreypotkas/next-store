...
const AppLeftSideBarItem = (props: AppMenuItemProps) => {
    ...
    <CSSTransition timeout={{ enter: 1000, exit: 450 }} classNames="layout-submenu" in={props.root ? true : active} key={item!.label}>
      <ul>
        {item!.items.map((child, i) => {
          return <AppLeftSideBarItem item={child} index={i} className={child.badgeClass} parentKey={key} key={child.label} />;
        })}
      </ul>
    </CSSTransition>
  );
...
export default AppLeftSideBarItem;
